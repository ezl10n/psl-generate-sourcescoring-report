...
public class SourceScoringTask extends Task<Void> {
	...
	protected Void call() throws Exception {
		int needCnt = 28;
		// input
		File[] lpuFiles = (new File(source)).listFiles((File f) -> !f
				.isDirectory() && f.getName().endsWith(".lpu"));
		...
		outer: for (IPslSourceList sourceList : sourceLists.toList()) {
			for (IPslSourceString sourceString : sourceList.getSourceStrings()) {
				if (sourceString == null || sourceString.getText()==null ||sourceString.getText().toLowerCase().trim().isEmpty()) {
					continue;
				}
				// just for example...
				for (String k : KEY_WORDS) {
					if (sourceString.getText().toLowerCase().trim().startsWith(k)) {
						needCnt--;
